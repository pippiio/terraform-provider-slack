		...
		Steps: []resource.TestStep{
			{
				...
				ExpectError: regexp.MustCompile(`slack_IDs must be in format \[C1A2B3C4D U1A2B3C4D\]`),
			},
		},
	})
}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("resource.slack_message.this", "slack_IDs", "C123456789"),
				),
			},
		},
	})
